    ...
    namespace Internal
    {
        ...
        public class PlanetRenderer : IEnumerable
        {
            ...
            private const string FragShader = ""
                ...
                + "void main()"
                + "{"
                + "    colour = vec4(0.5, 0.5, 0.0, 0.0); return;"
            ...
            public void Update()
            {
                CommandQueue.EnqueueCommand(new Action(delegate
                {
                    ...
                    while (ToCreate.TryDequeue(out pair))
                    {
                        ...
                        GL.BufferData<float>(BufferTarget.ArrayBuffer, (IntPtr)(pair.vertices.GetLength(0) * 6 * sizeof(float)), pair.vertices, BufferUsageHint.StaticDraw);
            ...
            public void Draw()
            {
                CommandQueue.EnqueueCommand(new Action(delegate
                {
                    ...
                    foreach (DrawData data in this)
                    {
                        ...
                        //Draw call
                        GL.DrawElements(PrimitiveType.Triangles, (int)Patch.I_NUM_INDICES, DrawElementsType.UnsignedInt, IntPtr.Zero);
                    }
                }));
            }

            public PlanetRenderer(ICommandQueue CommandQueue, Camera cam, PlanetImpl planet, int ColourTexture, int HeightMap, int NormalMap)
            {
                this.CameraInst = cam;
                ...
                CommandQueue.EnqueueCommand(new Action(delegate
                {
                    ...
                    GL.GetProgramInfoLog(Shader,  out log);
